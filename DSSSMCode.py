...
class DSSSM(nn.Module):
    ...
    def __init__(self, x_dim, y_dim, h_dim, z_dim, d_dim, n_layers, device, bidirection = False, bias = False,dataname=None):
        ...
        self.Transition_initial = torch.eye(self.d_dim,device = self.device) * (1-0.05*self.d_dim) + torch.ones((self.d_dim, self.d_dim),device = self.device) * 0.05
        ...
        if self.bidirection:
            self.rnn_backward = nn.GRU(y_dim + h_dim, int(h_dim/2), n_layers, bidirectional = self.bidirection)
        else:
            self.rnn_backward = nn.GRU(y_dim + h_dim, h_dim, n_layers, bidirectional = self.bidirection)

    def TransitionMatrix(self):
        
        if self.dataname == 'Sleep':
            Transition = self.dprior(self.Transition_initial)*0.2 + torch.eye(self.d_dim,device = self.device)*0.8
        else:
            Transition = self.dprior(self.Transition_initial)/2 + torch.eye(self.d_dim,device = self.device)/2
    ...
    def forward(self,x,y):
        ...
        if self.bidirection:
            A0 = torch.zeros((self.n_layers*2, x.size(1), int(self.h_dim/2)), device = self.device)
        else:
            A0 = torch.zeros((self.n_layers, x.size(1), self.h_dim), device = self.device)
        ...
        all_d_posterior.append(torch.ones((x.size(1),self.d_dim),device = self.device)/self.d_dim)
        ...
        for t in range(x.size(0)):
            ...
            d_posterior = 0 
            for i in range(self.d_dim):
                d_posterior_list.append(self.dposterior_list[i](output_backward[x.size(0)-t-1])) #batch*2
            ...
            for i in range(self.d_dim):
                z_prior_list.append(self.ztrainsition_list[i](torch.cat([output_forward[t],all_z_t_sampled[t]],1)))
                ...
                z_posterior_list.append(self.zposterior_list[i](torch.cat([output_backward[x.size(0)-t-1],all_z_t_sampled[t]],1)))
            ...
            for i in range(self.d_dim):
                kld_category_loss += torch.sum(self._kld_category(d_posterior_list[i], Transition[i:(i+1),:]) * all_d_posterior[-2][:,i])
...
#             print(self._kld_category(d_posterior_list[i], Transition[i:(i+1),:]))
    ...
    def _forecastingMultiStep(self, x, y, step = 1, S=1):
        ...
        with torch.no_grad():
            ...
            for s in range(S):
                ...
                kld_gaussian_loss, kld_category_loss, nll_loss,(all_z_posterior_mean, all_z_posterior_std),(all_y_emission_mean, all_y_emission_std),all_d_t_sampled_plot,z_t_sampled,all_d_posterior,all_d_t_sampled  = self.forward(x,y)
                ...
                for t in range(step):
                    ...
                    samples = torch.distributions.Categorical(d_prior).sample().type(torch.LongTensor)
                    ...
                    z_prior_std = 0     
                    for i in range(self.d_dim):                     
                        z_prior_list.append(self.ztrainsition_list[i](torch.cat([h_forward.squeeze(0),z_t_sampled[-1]],1)))
                        z_prior_mean_list.append(self.ztrainsition_mean_list[i](z_prior_list[i]))
                        z_prior_std_list.append(self.ztrainsition_std_list[i](z_prior_list[i]))
                        z_prior_mean += z_prior_mean_list[i] * d_t_sampled[:,i:(i+1)]
                        z_prior_std += z_prior_std_list[i] * d_t_sampled[:,i:(i+1)]
                    ...
                    for i in range(self.d_dim): 
                        ...
                        y_emission_std_list.append(self.yemission_std_list[i](
                            y_emission_list[i]))
                        y_emission_mean += y_emission_mean_list[i] * d_t_sampled[:,i:(i+1)]
                        y_emission_std += y_emission_std_list[i] * d_t_sampled[:,i:(i+1)]
                    all_y_emission_mean.append(y_emission_mean)
                    all_y_emission_std.append(y_emission_std)
                    y_t = torch.distributions.Normal(y_emission_mean,y_emission_std).sample().unsqueeze(0)
    ...
    #all_y_emission_mean,all_y_emission_std,d_t_sampled_plot,z_t_sampled,all_d_posterior,all_z_posterior_mean,all_z_posterior_std
...
#     def _forecastingMultiStep(self,x,y,step=1):

#         kld_gaussian_loss, kld_category_loss, nll_loss,(all_z_posterior_mean, all_z_posterior_std),(all_y_emission_mean, all_y_emission_std),d_t_sampled_plot,z_t_sampled,all_d_posterior = self.forward(x,y)
...
#             # d prior
#             d_prior = torch.mm(self._one_hot_encode(d_t_sampled_plot[-1],self.d_dim),Transition)
#             #print("d_prior:",d_prior)
#             all_d_posterior.append(d_prior[:,1])

#             # sample d from the prior
#             samples = torch.distributions.Bernoulli(d_prior[:,1]).sample().type(torch.LongTensor)
#             d_t_sampled_plot.append(samples)
#             ###
#             d_t_sampled = self._one_hot_encode(samples,self.d_dim)
#             #print("d_t_sampled:",d_t_sampled)
...
#             for i in range(self.d_dim):                     
#                 z_prior_list.append(self.ztrainsition_list[i](torch.cat([h_forward.squeeze(),z_t_sampled[t]],1)))
#                 z_prior_mean_list.append(self.ztrainsition_mean_list[i](z_prior_list[i]))
#                 z_prior_std_list.append(self.ztrainsition_std_list[i](z_prior_list[i]))
#                 z_prior_mean += z_prior_mean_list[i] * d_t_sampled[:,i:(i+1)]
#                 z_prior_std += z_prior_std_list[i] * d_t_sampled[:,i:(i+1)]
...
#             ## y emission
#             #y_emission = self.yemission(torch.cat([h_forward.squeeze(),z_t_sampled[t+1]], 1))
#             y_emission = self.yemission(torch.cat([h_forward.squeeze(),z_t_sampled[-1]], 1))
#             y_emission_mean = self.yemission_mean(y_emission)
#             y_emission_std = self.yemission_std(y_emission)

#             all_y_emission_mean.append(y_emission_mean)
#             all_y_emission_std.append(y_emission_std)
#             forecast_x.append(torch.distributions.Normal(y_emission_mean,y_emission_std).sample().unsqueeze(0))

#         return forecast_x,all_y_emission_mean,all_y_emission_std,d_t_sampled_plot,z_t_sampled,all_d_posterior,all_z_posterior_mean,all_z_posterior_std
...
def train(model,optimizer,trainX,trainY,epoch,batch_size,n_epochs,status="train"):
    ...
    all_d_t_sampled_plot,all_z_t_sampled,loss,all_d_posterior,all_z_posterior_mean = test(model,trainX,trainY,epoch,"train")
...
def test(model,testX,testY,epoch,status="test"):
    ...
    with torch.no_grad():
        ...
        kld_gaussian_loss,kld_category_loss, nll_loss,(all_z_posterior_mean, all_z_posterior_std),(all_y_emission_mean, all_y_emission_std),all_d_t_sampled_plot,all_z_t_sampled,all_d_posterior,all_d_t_sampled = model(testX,testY)
        ...
        print('{} Epoch:{}\t KLD_Gaussian Loss: {:.6f}, KLD_Category Loss: {:.6f}, NLL Loss: {:.6f}, Loss: {:.4f}'.format(status,epoch,kld_gaussian_loss_total/size,kld_category_loss_total/size,nll_loss_total/size,loss/size))
        
        #size = (batch,timestep,dim)
        all_d_t_sampled = torch.stack(all_d_t_sampled).cpu().numpy().transpose((1, 0, 2))
        all_d_t_sampled_plot = torch.stack(all_d_t_sampled_plot).cpu().numpy().transpose((1, 0, 2))
        all_z_t_sampled = torch.stack(all_z_t_sampled).cpu().numpy().transpose((1, 0, 2))
        all_d_posterior = torch.stack(all_d_posterior).cpu().numpy().transpose((1, 0, 2))
        all_z_posterior_mean = torch.stack(all_z_posterior_mean).cpu().numpy().transpose((1, 0, 2))
